        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                for c, r in list(zip(dialog['retrieved_contexts'], dialog['retrieved_resp'])):
                    ...
                    input_context_prompt += f"System: {r}" + self.prompt_tokenizer.sep_token
                
                # if '<movie>' in input_context:
                #     print(input_context)
                #     assert 1==0
                    
                #### use retrieved_contexts and un_mask_retrieved_resps to enrich the prompt.

                demonstration_ids = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(input_context))
                demonstration_ids = demonstration_ids[:self.context_max_length]
                ...
                context_ids = demonstration_ids + context_ids
...
class CRSConvDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        # input_batch['context']['labels'] = input_batch['resp']
