    ...
    def prepare_data(self, data_file):
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                # for c, r in list(zip(dialog['retrieved_contexts'], dialog['retrieved_resp'])):
                ...
                #         text2 += utt
                #         text2 += self.prompt_tokenizer.sep_token

                #     input_context +=  f"System: {r}" + self.tokenizer.eos_token
                #     input_context_prompt +=  f"System: {r}" + "<SEP>"

                # context = context + self.tokenizer.eos_token + input_context + self.tokenizer.eos_token
                # prompt_context = prompt_context + self.prompt_tokenizer.sep_token + input_context_prompt + self.prompt_tokenizer.sep_token
                ...
                # input_context += self.tokenizer.eos_token
                context_ids = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(context))
                context_ids = context_ids[-self.max_length:]
                # demonstration_ids = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(input_context))
                # demonstration_ids = demonstration_ids[:self.max_length]  
