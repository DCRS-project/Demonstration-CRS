...
class PromptGPT2forCRS(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.init_weights()
        self.prompt_max_length = 50

        # Model parallel
        self.model_parallel = False
        self.device_map = None
        self.gate_norm = nn.Linear(config.n_embd * 2, 1)
    ...
    def forward(
        ...
        if rec:
            ...
            ### get the last hidden state of the PLM.
            rec_logits = hidden_states[range(batch_size), sequence_lengths]  # (bs, hidden_size)
            # uc_gate = F.sigmoid(self.gate_norm(torch.cat([rec_logits, retrieved_vector], dim = -1)))
            # rec_logits = uc_gate * rec_logits + (1 - uc_gate) * retrieved_vector

            # rec_logits = retrieved_vector
            ### compute the ranking 
            if rec_labels is not None and not is_test:
                rec_logits @= entity_embeds.T  # (bs, n_item)
                # rec_logits *= retrieved_vector @ entity_embeds.T
        ...
        if conv:
            ...
            lm_logits = lm_logits[:, 3 * self.prompt_max_length:]
