    ...
    def __call__(self, data_batch):
        ...
        if self.gen:
            ...
            for data in data_batch:
                ...
                prompt_batch['input_ids'].append(data['prompt'])
                entity_batch.append(data['entity'])
                resp_batch.append(data['resp'][:self.resp_max_length])
        ...
        if not self.gen:
            ...
            for resp in resp_batch:
                pad = [-100] * (self.resp_max_length - len(resp))
                resp += pad
                new_resp_batch.append(resp)
            input_batch['resp'] = torch.as_tensor(new_resp_batch, device=self.device)
        else:
