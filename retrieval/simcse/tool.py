...
class SimCSE(object):
    ...
    def __init__(self, model_name_or_path: str, 
        ...
        # self.model = torch.load(os.path.join(model_name_or_path, "model.pth"))
        # self.model.load_state_dict(torch.load(os.path.join(model_name_or_path, "model.pth")))
    ...
    def search(self, queries: Union[str, List[str]], 
        ...
        if not self.is_faiss_index:
            ...
            def pack_single_result(dist, idx):
                results = [(self.index["sentences"][i], i, s) for i, s in zip(idx, dist) if s >= threshold]
                return results
